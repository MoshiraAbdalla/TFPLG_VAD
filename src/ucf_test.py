...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    Categorical_logits2=torch.zeros(0).to(device)
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            artificial_labels=gtlabels[i]
            if gtlabels[i]!=['A']:
                artificial_labels=[gtlabels[i][0]]
            else:
            ...
            #  Print or log the inference time per video
            ...
            Categorical_logits2=torch.cat([Categorical_logits2,  torch.tensor(element_logits2).to(device)], dim=0)
            
 
    avg_time = sum(video_times) / len(video_times)
    #print(f"\nAverage inference time per video: {avg_time:.4f} seconds")
    # Total inference time and frame count
...
if __name__ == '__main__':
    ...
    gt_labels=torch.zeros(0).to(device)
